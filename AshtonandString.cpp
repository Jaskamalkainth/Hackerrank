...
public:
    ...
    {
        for(int i = 0; i < len; i++)
            debug4(s.substr(SA[i],len-SA[i]+1),SA[i],iSA[i],lcp[i]);
    }
};
int main()
{
    ...
    {
        /*
         * "when concatenation of all substrings is done , not distinct"
        string s; cin >> s;
        int k; cin >> k;
        suffixArray sa(s);
        sa.SuffixSort();
        vector<ll> cumSA(s.size()+1);
        for(int i = 1; i <= s.size(); i++)
        {
          ll numSubstr = (s.size()-sa.SA[i-1]);
          numSubstr = (numSubstr * (numSubstr + 1))/2;
          cumSA[i] = cumSA[i-1] + numSubstr;    
        }
        auto pos = lower_bound(cumSA.begin(),cumSA.end(),k)-cumSA.begin();
        int finalk = k - cumSA[pos-1];
        int sufLen = (s.size()-sa.SA[pos-1]);
        vector<ll> cumHelp;
        cumHelp.push_back(0);
        for(int i = 1; i <= s.size(); i++)
            cumHelp.push_back(i);
        for(int i = 1; i <= s.size(); i++)
          cumHelp[i] += cumHelp[i-1];    
        auto posFinal = lower_bound(cumHelp.begin(),cumHelp.end(),finalk)-cumHelp.begin();
        posFinal = finalk - cumHelp[posFinal-1];
        cout << s[sa.SA[pos-1]+ posFinal-1] << "\n";
...
*/
        ...
        for(int i = 1; i <= s.size(); i++)
        {
            ll numSubstr = (s.size()-sa.SA[i-1]);
            numSubstr = (numSubstr * (numSubstr + 1))/2;
            ll commonSubstr = sa.lcp[i-1];
            commonSubstr = (commonSubstr * (commonSubstr + 1))/2;
            cumSA[i] = cumSA[i-1] + (numSubstr - commonSubstr);
        }
        //sa.PrintSA();
        //debug(cumSA);
        auto pos = lower_bound(cumSA.begin(),cumSA.end(),k)-cumSA.begin();
        //debug(pos);
        ll finalk = k - cumSA[pos-1];
        //debug(finalk);
        ll lcpp = sa.lcp[pos-1];
        ll sufLen = (s.size()-sa.SA[pos-1]);
        vector<ll> cumHelp;
        cumHelp.push_back(0);
        //debug(lcpp);
        for(int i = 1; i <= s.size(); i++)
            cumHelp.push_back(lcpp+i);
        for(int i = 1; i <= s.size(); i++)
            cumHelp[i] += cumHelp[i-1];    
        auto posFinal = lower_bound(cumHelp.begin(),cumHelp.end(),finalk)-cumHelp.begin();
        posFinal = finalk - cumHelp[posFinal-1];
